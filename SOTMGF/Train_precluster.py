...
from model import TransformerModel, DECT,GDECT
...
##########################################################################
def precluster(adata, label_true):
    args={
    'seed':2023,
    'lr':0.000001,
    'epochs':1000,
    ...
    X = adata.obsm['X_tem']
    label_pre = np.array(adata.obs['label_tem']).reshape(3639,1)
...
###train()
    ...
    model = DECT(input_size=X.shape[1],  # 输入维度
                        num_classes=args['ncluster'], embedding_size = X.shape[1]).to(device)  # 输出维度
    ...
    with torch.no_grad():
        ...
        kmeans = KMeans(n_clusters=args['ncluster'], n_init=20)
...
#         np.savetxt("./output/precluster/pred_precluster.csv", preds_k,delimiter=',')
    ...
    adata.obs['label_tem'] = predicted_T.cpu().detach().numpy()
